...
function mfn_is_blocks( $vb = false ){

	global $post;

	if( !empty($post->ID) && get_post_type($post->ID) == 'template' && get_post_meta($post->ID, 'mfn_template_type', true) && in_array( get_post_meta($post->ID, 'mfn_template_type', true), array('header', 'footer', 'megamenu') ) ){
		return;
	}

	if( $vb || (!empty($_GET['visual']) && 'iframe' == $_GET['visual']) ){
