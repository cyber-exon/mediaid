...
if( ! class_exists('Mfn_Builder_Items') )
{
  class Mfn_Builder_Items {
    ...
    public static function item_blog_teaser( $fields ){
		...
		public static function blocks( $item, $vb = false ){
			...
			$item_fields = new Mfn_Builder_Fields();
			$item_fields = $item_fields->get_item_fields( $item_type );
			...
			// live view for items with inline editor

			// if( in_array( $item_type, ['column'] ) ){
			// 	$type = 'item_'. $item['type'];
			//
			// 	self::$type( $item['attr'], $vb );
			// 	return $output;
			// }
